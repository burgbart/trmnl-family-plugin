...
def render(device_profile: DeviceProfile | str, data: dict) -> str:
    ...
    if device_profile.template_filename is None:
        raise ValueError(
            f"Device '{device_profile.name}' has no template configured; "
            "cannot render."
        )

    template = _env.get_template(device_profile.template_path)
